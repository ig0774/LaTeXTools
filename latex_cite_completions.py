...
class CiteFillAllHelper(FillAllHelper):
    ...
    def get_auto_completions(self, view, prefix, line):
        ...
        if prefix:
            ...
            completions = [
                c for c in completions if lower_prefix in "{0} {1} {2}".format(
                    c['keyword'].lower(),
                    c['title'].lower(),
                    c['author'].lower()
                )
        ...
        formatter = Formatter()
        completions = [(
            formatter.vformat(cite_autocomplete_format, (), completion),
            completion['keyword']) for completion in completions
        ]
    ...
    def get_completions(self, view, prefix, line):
        ...
        for completion in completions:
            formatted_completions.append(
                [formatter.vformat(s, (), completion)
